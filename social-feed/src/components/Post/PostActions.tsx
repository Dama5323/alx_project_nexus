    ...
    <div className="post-actions">
      <Tooltip content="Reply">
        ...
        >
          ...
          {comments > 0 && <span className="post-actions__count">{formatNumber(comments)}</span>}
        </button>
      </Tooltip>

      <Tooltip content={isReposted ? 'Undo repost' : 'Repost'}>
        <button
          className={`post-actions__button ${isReposted ? 'post-actions__button--active-repost' : ''}`}
        ...
        >
          ...
          {reposts > 0 && <span className="post-actions__count">{formatNumber(reposts)}</span>}
        </button>
      </Tooltip>

      <Tooltip content={isLiked ? 'Unlike' : 'Like'}>
        <button
          className={`post-actions__button ${isLiked ? 'post-actions__button--active-like' : ''}`}
        ...
        >
          ...
          {likes > 0 && <span className="post-actions__count">{formatNumber(likes)}</span>}
        </button>
      </Tooltip>
      ...
      <Tooltip content="Share">
        ...
        >
          ...
          {shares > 0 && <span className="post-actions__count">{formatNumber(shares)}</span>}
        </button>
      </Tooltip>

      <Tooltip content={isSaved ? 'Remove from saved' : 'Save'}>
        <button
          className={`post-actions__button ${isSaved ? 'post-actions__button--active-save' : ''}`}
